...
public final class ApiRepository {
 ...
 public void uploadImage(Context context,String path,Uri uri,Callback callback){executor.execute(()->{try{if(base().isEmpty())throw new IllegalStateException("Set your backend address in Account → Connection");String mime=context.getContentResolver().getType(uri);if(!"image/jpeg".equals(mime)&&!"image/png".equals(mime))throw new IllegalArgumentException("Choose a JPEG or PNG image");byte[] bytes;try(var in=context.getContentResolver().openInputStream(uri);var out=new ByteArrayOutputStream()){if(in==null)throw new FileNotFoundException("Image could not be opened");byte[] buf=new byte[8192];int n;while((n=in.read(buf))!=-1){out.write(buf,0,n);if(out.size()>5*1024*1024)throw new IllegalArgumentException("Image must be below 5 MB");}bytes=out.toByteArray();}var multipart=new MultipartBody.Builder().setType(MultipartBody.FORM).addFormDataPart("file","parlour."+("image/png".equals(mime)?"png":"jpg"),RequestBody.create(bytes,MediaType.get(mime))).build();var r=new Request.Builder().url(base()+path).post(multipart);String t=token();if(!t.isEmpty())r.header("Authorization","Bearer "+t);try(var response=http.newCall(r.build()).execute()){String raw=response.body()==null?"{}":response.body().string();Object parsed=new JSONTokener(raw).nextValue();if(!response.isSuccessful()){String error=parsed instanceof JSONObject o?o.optString("message","Upload failed ("+response.code()+")"):"Upload failed ("+response.code()+")";throw new IllegalStateException(error);}callback.done(parsed,null);}}catch(Exception e){callback.done(null,e.getMessage()==null?"Upload failed":e.getMessage());}});}
