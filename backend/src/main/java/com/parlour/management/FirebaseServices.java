...
public class FirebaseServices {
 ...
 var bucket=StorageClient.getInstance(app).bucket();String key="parlours/"+pid+"/"+UUID.randomUUID(),token=UUID.randomUUID().toString();var blob=bucket.create(key,bytes,mime);blob.toBuilder().setMetadata(Map.of("firebaseStorageDownloadTokens",token)).build().update();String url="https://firebasestorage.googleapis.com/v0/b/"+bucket.getName()+"/o/"+URLEncoder.encode(key,StandardCharsets.UTF_8)+"?alt=media&token="+token;try{long imageId=db.insert("parlour_images",map("parlour_id",pid,"image_url",url,"type","GALLERY","sort_order",db.count("SELECT COUNT(*) FROM parlour_images WHERE parlour_id=?",pid)));return map("image_id",imageId,"image_url",url);}catch(Exception e){blob.delete();throw e;}
 }
}
